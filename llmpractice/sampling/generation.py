...
def update_stop_tokens_tracker(
        tokens: torch.Tensor, stop_tokens: torch.Tensor,
        stop_token_reached: torch.Tensor) -> torch.Tensor:
...
def get_causal_mask_from_padding_mask(
    ...
    mask.narrow(2, 0,
                seq_len).mul_(padding_mask[:, None, :].expand(-1, seq_len, -1))
...
def generate(
    ...
    *,
    ...
    # length otherwise
    max_seq_len = (total_response_length if not incremental_decoding else
    ...
    if not padding_masks.all():
        ...
        padding_masks = torch.nn.functional.pad(padding_masks,
                                                (0, max_generated_tokens),
                                                value=True)

        # generate the full causal mask for the whole padding mask with padding ignored
        masks = get_causal_mask_from_padding_mask(padding_masks,
        ...
        input_pos = torch.arange(0,
                                 total_response_length,
                                 device=generated_tokens.device).unsqueeze(0)
    ...
    next_token_fn = (compiled_generate_next_token
                     if compiled_generate_next_token is not None else
                     generate_next_token)
    ...
    if stop_tokens is not None:
        ...
        generated_logits *= stop_token_mask[:, -generated_logits.shape[1]:,
                                            None]
