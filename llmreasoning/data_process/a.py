...
class MetaData(TypedDict):
    ...
    Attributes:
        total_answers (int): The total number of answers/CoTs for a given question.
        correct_count (int): The number of correct CoTs.
        cots_len_list (List[int]): A list of token lengths for each CoT.
        avg_cot_token_len (float): The average token length of all CoTs.
        max_cot_token_len (int): The maximum token length among all CoTs.
        min_cot_token_len (int): The minimum token length among all CoTs.
    """
    total_answers: int
    correct_count: int
...
def main() -> None:
    ...
    # Initialize data processor and handle tokenizer loading errors
    try:
    ...
    # Generate SFT data entries in parallel
    logger.info('Generating SFT data...')
    # Use partial to pass the instance method `generate_sft_data` with its `self` context
    # This allows it to be used by the `map` function as a standard callable.
    generate_func = data_processor.generate_sft_data
    ...
    # Flatten the data into a single SFT dataset
    logger.info('Flattening SFT data...')
    # The `mapped_dataset` is a dataset of lists. We must flatten it.
    # We iterate over the dataset and its sub-lists, filtering out any empty entries.
