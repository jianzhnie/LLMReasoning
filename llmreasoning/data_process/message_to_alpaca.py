...
def convert_multi_turn_to_alpaca(
    ...
    for conv in conversations:
        ...
        if first_user_msg:
            ...
            output = '\n'.join(
                assistant_responses) if assistant_responses else ''
...
def main():
    ...
    if args.from_hf:
        ...
        conversations = dataset['train'] if 'train' in dataset else list(
            dataset.values())[0]
    ...
    # 转换数据
    if args.mode == 'full':
        alpaca_data = convert_multi_turn_to_alpaca(conversations,
                                                   args.system_prompt)
    else:
        alpaca_data = convert_single_turn_to_alpaca(conversations,
                                                    args.system_prompt)
    ...
    {
        'uid':
        '1b6d912-8135-4f23-b704-2ceea8675617',
        'license':
        'CC BY 4.0',
        'generator':
        'Owen3-235B-A22B',
        ...
        'messages': [{
            ...
            'role': 'assistant',
            'content':
            "Understood. I'm ready to proceed with the activity. Please ask the first question.",
            'tool_calls': []
        }],
        'metadata': {
