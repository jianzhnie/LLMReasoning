...
| DPO      | 对数概率比作为隐式奖励       | 是               | $- \log \sigma\left(\beta \left(\log \frac{\pi_\theta(y_w | x)}{\pi_{\text{ref}}(y_w | x)} - \log \frac{\pi_\theta(y_l                | x)}{\pi_{\text{ref}}(y_l | x)}\right)\right)$ | 中 | 稳定、广泛应用，但需额外维护参考模型 |
